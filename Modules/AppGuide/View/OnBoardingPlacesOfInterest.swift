...
class OnBoardingPlacesOfInterest: UIViewController {
    ...
    @objc func handleSwipeGesture(gesture: UIGestureRecognizer) {
        ...
        if let swipeGesture = gesture as? UISwipeGestureRecognizer {
            if swipeGesture.direction == .left {
                let onBoardingAutomatedFare = OnBoardingAutomatedFare.instantiate(appStoryboard: .onBoarding)
                self.navigationController?.pushViewController(onBoardingAutomatedFare, animated: true)
