...
class OnBoardingKaprptNetwork: UIViewController {
    ...
    @objc func handleSwipeGesture(gesture: UIGestureRecognizer) {
        ...
        if let swipeGesture = gesture as? UISwipeGestureRecognizer {
            if swipeGesture.direction == .left {
                let onBoardingPlacesOfInterest = OnBoardingPlacesOfInterest.instantiate(appStoryboard: .onBoarding)
                self.navigationController?.pushViewController(onBoardingPlacesOfInterest, animated: true)
