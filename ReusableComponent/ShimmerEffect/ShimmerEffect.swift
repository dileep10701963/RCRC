...
@objc extension UITableView: ListLoadable {
    public func ld_visibleContentViews() -> [UIView] {
        return (self.visibleCells as NSArray).value(forKey: Constants.shimmerContentView) as! [UIView]
    }
}
...
@objc extension UIColor {
    static func backgroundFadedGrey() -> UIColor {
        return UIColor(red: (240.0/255.0), green: (241.0/255.0), blue: (242.0/255.0), alpha: 1)
    }

    static func gradientFirstStop() -> UIColor {
        return  UIColor(red: (235.0/255.0), green: (238.0/255.0), blue: (235.0/255.0), alpha: 1.0)
    }
...
@objc extension UIView {
    func boundInside(_ superView: UIView) {
        ...
        superView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-0-[subview]-0-|", options: NSLayoutConstraint.FormatOptions(), metrics: nil, views: [Constants.shimmerSubview: self]))
        superView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:|-0-[subview]-0-|", options: NSLayoutConstraint.FormatOptions(), metrics: nil, views: [Constants.shimmerSubview: self]))
    }
}
...
@objc extension UIView {
    ...
    func configureAndAddAnimationToGradient(_ gradient: CAGradientLayer) {
        ...
        let gradientAnimation = CABasicAnimation(keyPath: Constants.shimmerLocations)
        ...
        gradient.add(gradientAnimation, forKey: Constants.shimmerLocations)
